...
size: The size is a float that is randomly assigned between size_min and size_max, two variables that never change during runtime. 

value: The value is calculated based on the previous two fields, the rarity and the size. The formulas is as follows:
...
reeling: For the actual reeling, based on the player’s reel_skill, a conditional is used to determine if each instance reeled a fish or not. The conditional used is: 
...
This should yield a roughly 50\% chance for a safe or a bust. Because there is no replacement, it is not exactly 50\% but it should hover around that value. This strategy may lead to more overall busts from the rare fish, but it should produce higher values on average. Despite the fish busting more frequently, it should force the player to play more aggressively, as playing too passively would result in them consistently losing. 
...
The fish’s victory conditions are far smaller than before, so they are more than likely to bust. As mentioned before, the average hand has a sum of 14.6~, so although the fish has an advantage, hitting every time will be a matter of luck as opposed to skill. However, powerups should be able to help mitigate this issue. 

Behavior Tree:

TODO:

![Reeling_logic](project_contribution_visuals/IMG_0460.jpg)

![Common fish](project_contribution_visuals/IMG_0461.jpg)

![Rare fish](project_contribution_visuals/IMG_0462.jpg)

![Super Rare fish](project_contribution_visuals/IMG_0463.jpg)
...
The fishing related information is all in:
fish_logic.gd

The fish blackjack behavior is in:
blackjack.gd
...
The tutorial is in Tutorial_Manager.gd, and the hints are in Hints.gd. To enable the tutorial, please go to WorldUI.gd and turn dev_mode = false to start the tutorial. 
...
Helped implement debt payments within one of the NPCs.
